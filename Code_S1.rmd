...
ex2d_data<-read.table("Data_table_s16.txt", header = T)
...
ex2b_data<-ex2b_data[is.nan(ex2b_data$plost)==F,]
#try4 data processing 
ex2d_data$ex<-"D"
ex2d_data$plost<-ex2d_data$negative/ex2d_data$Counts
ex2d_data<-ex2d_data[complete.cases(ex2d_data),]
ex2d_data<-ex2d_data[ex2d_data$Counts >= 30, ]
ex2d_data_filt<-ex2d_data[, c("date","Genotype", "Condition","Hour","Counts", "ilux","negative","ex","plost")]

##cobineB and C data
ex2_dataFULL<-rbind(ex2b_datafilt, ex2c_data)
ex2_dataFULL<-ex2_dataFULL[, c("date","Genotype", "Condition","Hour","Counts", "ilux","negative","ex","plost")]
ex2_dataFULL<-rbind(ex2_dataFULL, ex2d_data_filt)
ex2_dataFULL$Treatment<-paste0(ex2_dataFULL$Genotype, ex2_dataFULL$Condition)

testmeansamp <- aggregate(plost ~  Hour, ex2_dataFULL[ex2_dataFULL$Condition=="AMP",], mean)
testmeansamp$Condition<-"AMP"
testmeans <- aggregate(plost ~  Hour, ex2_dataFULL[ex2_dataFULL$Condition=="STANDARD",], mean)
testmeans$Condition<-"STANDARD"
testmeans<-rbind(testmeansamp,testmeans)
testmeans$plost<-round(testmeans$plost,digits = 2)

FIG_S6A<-ggplot(data = ex2_dataFULL, aes(x=as.factor(`Hour`) ,y=`plost`, fill=`Condition`))
#5x3
#7x3v2
FIG_S6A+ 
  geom_boxplot() +
  ...
  labs(title="Supplemental Figure 6A\nPlasmid loss over time",
       ...
       caption = "Boxplot shows data median, red dot shows mean value")+
  scale_fill_manual(name="Condition", values = c("gray","gray"), guide="none")+
  geom_point(position=position_jitterdodge(jitter.width=0, dodge.width = .7, seed = 1234), size = 2, shape=16, alpha = .6, aes(color=Condition))+
  geom_text(data = testmeans[testmeans$Condition=="AMP",],
            size = 2.5, nudge_x = -.2 ,
            aes(label = plost, y = plost+0.02))+
  geom_text(data = testmeans[testmeans$Condition=="STANDARD",], 
            size = 2.5, nudge_x = .2 ,
            aes(label = plost, y = plost+0.02))+
  geom_point(data = testmeans[testmeans$Condition=="AMP",],color= 'red', size = 2, shape=16, aes(y=`plost`, x=as.factor(`Hour`)))+
   geom_point(data = testmeans[testmeans$Condition=="STANDARD",],color= 'red', size = 2, shape=16, aes(y=`plost`, x=as.factor(`Hour`)))+
  theme(plot.title = element_text(hjust = 0.5), 
        axis.title.x = element_text(size = 10), 
        axis.title.y = element_text(size = 10),
        axis.text.x = element_text(size= 10), 
...
totmeansamp <- aggregate(negative ~  Hour, ex2_dataFULL[ex2_dataFULL$Condition=="AMP",], mean)
totmeansamp$Condition<-"AMP"
totmeans <- aggregate(negative ~  Hour, ex2_dataFULL[ex2_dataFULL$Condition=="STANDARD",], mean)
totmeans$Condition<-"STANDARD"
totmeans<-rbind(totmeansamp,totmeans)
totmeans$negative<-round(totmeans$negative,digits = 2)

FIG_S6B<-ggplot(data = ex2_dataFULL, aes(x=as.factor(`Hour`) ,y=`negative`, fill = `Condition`))
FIG_S6B+ 
  geom_boxplot()+ 
  xlab("hour")+
  ylab("total of colonies\nplasmid lost")+
  ggtitle("Supplemental Figure 6B\nPlasmid loss over time") +
  theme_bw() + 
  scale_fill_manual(name="Condition", values = c("gray","gray"), guide="none")+
  geom_point(position=position_jitterdodge(jitter.width=0, dodge.width = .7, seed = 1234), size = 2,shape=16, alpha = .6, aes(color=Condition))+
    geom_text(data = totmeans[totmeans$Condition=="AMP",],
            size = 2.5, nudge_x = -.2 ,
            aes(label = negative, y = negative+1.5))+
  geom_text(data = totmeans[totmeans$Condition=="STANDARD",], 
            size = 2.5, nudge_x = .2 ,
            aes(label = negative, y = negative+1.5))+
  geom_point(data = totmeans[totmeans$Condition=="AMP",],color= 'red', size = 2, shape=16, aes(y=`negative`, x=as.factor(`Hour`)))+
   geom_point(data = totmeans[totmeans$Condition=="STANDARD",],color= 'red', size = 2, shape=16, aes(y=`negative`, x=as.factor(`Hour`)))+
  theme(plot.title = element_text(hjust = 0.5), 
        axis.title.x = element_text(size = 10), 
        axis.title.y = element_text(size = 10),
        axis.text.x = element_text(size= 10), 
...
FIG_S6C<-ggplot(data=ex2_dataFULL, aes(x=as.factor(`Hour`), y=`plost`,fill=`Treatment`) )

FIG_S6C+
  geom_boxplot(outlier.size = .7, lwd=.3)+
  labs(title="Supplemental Figure 6C\nPlasmid loss over time\nfly feeding conditions",
       x="hour",
       y= "proportion of colonies\nplasmid lost")+
  geom_point(position=position_jitterdodge(jitter.width=0, dodge.width = .7, seed = 1234), size = 2,shape=16, alpha = .6, aes(color=Treatment))+
  scale_fill_manual(name="Genotype", values = c("gray","gray","gray", "gray"), guide="none")+
  scale_color_discrete(name="Treatment", breaks=c("G090AMP","G090STANDARD","G130AMP", "G130STANDARD"), labels=c("wt+AMP","wt","imd+AMP ", "imd"))+
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5), 
        axis.title.x = element_text(size = 10), 
        axis.title.y = element_text(size = 10),
...
```{r}
#count data 
ex2d_counts<-read.table("Data_table_s16.txt", header = T)
#flux data 
ex2d_flux<-read.table("Data_table_s17.txt", header = T)

#process data
ex2d_gather<-gather(ex2d_flux,"Column", "Flux", C1:C12)
ex2d_gather$Well_ID<-paste(ex2d_gather$Row, ex2d_gather$Column, sep="_" )
#assign unique ID to each flux measurement
ex2d_gather$ID<-paste(ex2d_gather$Well_ID, ex2d_gather$Genotype,ex2d_gather$Condition ,ex2d_gather$Hour, sep="" )
#foodsig<-ex2d_gather[ex2d_gather$Genotype=="EMPTY",]
#calculate CFU's perfly 
ex2d_counts$CFUpFly <-(ex2d_counts$Counts/200)*ex2d_counts$DF*250
ex2d_counts_filt<-ex2d_counts[complete.cases(ex2d_counts),]
#assign a unique ID to each CFU measurment that corresponds to each flux measurment
ex2d_counts_filt$ID<-paste(ex2d_counts_filt$Well_ID, ex2d_counts_filt$Genotype,ex2d_counts_filt$Condition ,ex2d_counts_filt$Hour, sep="" )

#regression
ex2d_complete<-merge(ex2d_counts_filt, ex2d_gather, by.x=16, by.y=8, all.x=T)
ex2d_complete<-setDT(ex2d_complete)[,c("Row.y","Hour.y", "Genotype.y", "Column.y"):=NULL]
ex2d_complete<-ex2d_complete[ex2d_complete$Counts >=30,]

#G130 standard
ex2d_130<-ex2d_complete[ex2d_complete$Genotype.x=="G130" & ex2d_complete$Condition.x=="STANDARD" ,]
ex2d_130REG<-lmodel2(log(Flux)~log(CFUpFly) , data=ex2d_130 , nperm = 99)
reg130<-paste("G130: y=",signif(ex2d_130REG[[3]][3,3],2),"x+",signif(ex2d_130REG[[3]][3,2],2),"CI [",signif(ex2d_130REG[[4]][3,4],2),",",signif(ex2d_130REG[[4]][3,5],2),"]")
reg130
#mreglog[[3]][3,2]
#G130 AMP
ex2d_130amp<-ex2d_complete[ex2d_complete$Genotype.x=="G130" & ex2d_complete$Condition.x=="AMP" ,]
ex2d_130REGamp<-lmodel2(log(Flux)~log(CFUpFly) , data=ex2d_130amp , nperm = 99)
reg130amp<-paste("G130: y=",signif(ex2d_130REGamp[[3]][3,3],2),"x+",signif(ex2d_130REGamp[[3]][3,2],2),"CI[",signif(ex2d_130REGamp[[4]][3,4],2),",",signif(ex2d_130REGamp[[4]][3,5],2) ,"]")
reg130amp
#G090 standard
ex2d_090<-ex2d_complete[ex2d_complete$Genotype.x=="G090" & ex2d_complete$Flux>0,]
ex2d_090$CFUpFly<-ex2d_090$CFUpFly+1
ex2d_090REG<-lmodel2(log(Flux)~log(CFUpFly) , data=ex2d_090 , nperm = 99)
reg090<-paste("G090: y=",signif(ex2d_090REG[[3]][3,3],2),"x+",signif(ex2d_090REG[[3]][3,2],2))
ex2d_complete$GT<-paste0(ex2d_complete$Genotype.x, ex2d_complete$Condition.x)

fluxcorrected<-ex2d_complete$Flux
fluxcorrected[fluxcorrected < 0 ]<-1
ex2d_complete$Flux<-fluxcorrected

FIG_s5<-ggplot(data = ex2d_complete[ex2d_complete$Genotype.x=="G130",], aes(x=`CFUpFly`,y=`Flux`, colour=as.factor(`Hour.x`), shape=`GT`)) 
# 5 x 3.5
FIG_s5 +
  geom_point(size = 5, alpha = .5, position=position_jitter(0.6)) +
  ylab("flux \n (p/sec)")+
  xlab("CFUs per fly ")+
  ggtitle("Supplemental Figure 7 \nimd Total Radiance vs Total CFUs")+
  scale_y_log10()+
  scale_x_log10()+
  scale_y_continuous(breaks= ax_breaks,labels=ax_labeles,trans='log')+
  scale_x_continuous(breaks= ax_breaks,labels=ax_labeles,trans='log')+
  scale_color_discrete(name="hour")+
  scale_shape_discrete(name="treatment", breaks=c("G130AMP", "G130STANDARD"), labels=c("imd+AMP", "imd"))+
  geom_hline(yintercept = 1000, linetype="dashed", color = "red")+
  theme_bw() + 
  annotate("text",x=1000000, y=800, label="Background Signal" )+
  theme(plot.title = element_text(hjust = 0.5), 
...
ex4c_g90_h0<-ex4c_cfu[ex4c_cfu$genotype=="G090" & ex4c_cfu$Hour==0,]
ex4c_g90_h115<-ex4c_cfu[ex4c_cfu$genotype=="G090" & ex4c_cfu$Hour==11.5,c("radiance","ID")]
colnames(ex4c_g90_h115)[1]<-"radiance11.5"
ex4c_g90_h0<-merge( ex4c_g90_h0, ex4c_g90_h115, by.x=7, by.y= 2)
ex4c_g90_h0test<-cor.test(ex4c_g90_h0$radiance, ex4c_g90_h0$radiance11.5)

FIGS8a <-ggplot(data = ex4c_g90_h0, aes(x=`radiance11.5`,y=`radiance`))

FIGS8a +
  geom_point()+
  ylab("initial fLux (p/s)")+
  xlab("final flux (p/s")+
  annotate("text", y=9990000, x=50000000,label=paste("Pearson's product-moment\n correlation\nR=",signif(ex4c_g90_h0test$estimate,2),"\np=",signif(ex4c_g90_h0test$p.value,2) ,"\n CI=",signif(ex4c_g90_h0test$conf.int[1],2),",",signif(ex4c_g90_h0test$conf.int[2],2) ),size=3)+
  #annotate("text", x="G130", y=60000000,label=paste("n.s."),size=3)+
  ggtitle("Supplemental Figure 8a\nWT initial vs final dose ")+
  scale_y_continuous(breaks= ax_breaks, labels=ax_labeles, trans='log',limits=c(100000,100000000))+
  scale_x_continuous(breaks= ax_breaks,labels=ax_labeles,trans='log',limits= c(100000,100000000))+
  theme_bw()+
  theme(axis.title.x = element_text(size = 10), 
        axis.title.y = element_text(size = 10),
        axis.text.x = element_text(size= 10), 
...
##g130
ex4c_g130_h0<-ex4c_cfu[ex4c_cfu$genotype=="G130" & ex4c_cfu$Hour==0,]
ex4c_g130_death<-ex4c_cfu[ex4c_cfu$genotype=="G130" & ex4c_cfu$firstdeath==T ,c("radiance","ID")]
colnames(ex4c_g130_death)[1]<-"radiancedeath"
ex4c_g130_h0<-merge( ex4c_g130_h0, ex4c_g130_death, by.x=7, by.y= 2)
ex4c_g130_h0test<-cor.test(ex4c_g130_h0$radiance, ex4c_g130_h0$radiancedeath)

FIGS8b <-ggplot(data = ex4c_g130_h0, aes(x=`radiancedeath`,y=`radiance`))

FIGS8b +
  geom_point()+
  ylab("initial fLux (p/s)")+
  xlab("flux at death (p/s")+
  annotate("text", y=70000000, x=15000000,label=paste("Pearson's product-moment\n correlation\nR=",signif(ex4c_g130_h0test$estimate,2),"\np=",signif(ex4c_g130_h0test$p.value,2) ,"\n CI=",signif(ex4c_g130_h0test$conf.int[1],2),",",signif(ex4c_g130_h0test$conf.int[2],2) ),size=3)+
  ggtitle("Supplemental Figure 8b\nimd10191 initial vs final dose ")+
  scale_y_continuous(breaks= ax_breaks, labels=ax_labeles, trans='log',limits=c(10000000,100000000))+
  scale_x_continuous(breaks= ax_breaks,labels=ax_labeles,trans='log',limits= c(10000000,100000000))+
  theme_bw()+
  theme(axis.title.x = element_text(size = 10), 
        axis.title.y = element_text(size = 10),
        axis.text.x = element_text(size= 10), 
...
## g130 timer after crossing threshhold
###function assumes firstdeath (function deathlabeler) and radiancedeath is already 
threshcross<-function(g130data, deathmeanflux){
  tempIDs<-unique(g130data$ID)
  outnum<-as.character()
  #colnames(outdf)<-colnames(g130data)
  for ( i in 1:length(tempIDs)){
    temp_fly<-tempIDs[i]
    temp_data<-g130data[g130data$ID==temp_fly,]
    temp_thresh<-temp_data[temp_data$radiance >= deathmeanflux,]
    if (nrow(temp_thresh)==0 ){
      next()
    } else{
      firstthresh<-rownames(temp_thresh[1,])
      outnum<-append(outnum,firstthresh)
    }
  }
  outnumlab<-rownames(g130data) %in% outnum
  return(outnumlab)
}

ex4c_g130<-ex4c_cfu[ex4c_cfu$genotype=="G130",]
#sd(ex4c_cfu[ex4c_cfu$death==TRUE & ex4c_cfu$firstdeath==TRUE,c("radiance")])
ex4c_g130$cross<-threshcross(ex4c_g130, (death_mean-7334353))
##assumes threshcross has been calculated

g130datathresh<-ex4c_g130
###
thresh2death<-function(g130datathresh){
  tempIDs<-unique(g130datathresh$ID)
  outdf<-g130datathresh[g130datathresh$Hour==0,]
  outtime<-as.numeric()
  #colnames(outdf)<-colnames(g130data)
  for ( i in 1:length(tempIDs)){
    temp_fly<-tempIDs[i]
    temp_data<-g130datathresh[g130datathresh$ID==temp_fly,]
    temp_time<-temp_data[temp_data$cross==T | temp_data$firstdeath==T,]
    if (nrow(temp_time)==1 ){
      outtime<-append(outtime,.1)
    } else{
      temp_length<-temp_data[temp_data$firstdeath==T,c("Hour")]-temp_data[temp_data$cross==T,c("Hour")]
      outtime<-append(outtime,temp_length)
    }
  }
  outdf<-cbind(outdf,outtime)
  return(outdf)
}

ex4c_g130_time<-thresh2death(ex4c_g130)
ex4c_g130_timetest<-cor.test(ex4c_g130_time$radiance, ex4c_g130_time$outtime)

FIGS8c <-ggplot(data = ex4c_g130_time, aes(y=`radiance`,x=`outtime`))

FIGS8c +
  geom_point()+
  ylab("initial fLux (p/s)")+
  xlab("time to death after hitting threshhold")+
  annotate("text", y=70000000, x=6.9,label=paste("Pearson's product-moment\n correlation\nR=",signif(ex4c_g130_timetest$estimate,2),"\np=",signif(ex4c_g130_timetest$p.value,2) ,"\n CI=",signif(ex4c_g130_timetest$conf.int[1], 2) ,",",signif(ex4c_g130_timetest$conf.int[2],2) ),size=3)+
  ggtitle("Supplemental Figure 8c\nimd10191 initial flux vs time to death ")+
  scale_y_continuous(breaks= ax_breaks, labels=ax_labeles, trans='log',limits=c(10000000,100000000))+
  #$scale_x_continuous(breaks= ax_breaks,labels=ax_labeles,trans='log',limits= c(10000000,100000000))+
  theme_bw()+
  theme(axis.title.x = element_text(size = 10), 
...
fig5b<-fig5b[fig5b$Death==F,]

tempdata<-fig5b[fig5b$Hour<48,]
temptest_out<-runningt(tempdata,"genotype","Hour")
temp36<-fig5b[fig5b$Hour==36,]
t.test(temp36[temp36$genotype=="G130",c("radiance")],temp36[temp36$genotype=="G090",c("radiance")] )
...
```{r}
FIG6B<-ggplot(data=ex4e_130_re, aes(x= `Hour`, y=`radiance`, color=`clustlabs`, group=`ID`, shape=`Death`))
...
```{r}
ex4e_g130_h0<-ex4e_130_re[ex4e_130_re$genotype=="G130" & ex4e_130_re$Hour==0,]
ex4e_g130_death<-ex4e_130_re[ex4e_130_re$genotype=="G130" & ex4e_130_re$firstdeath==T ,c("radiance","ID")]
colnames(ex4e_g130_death)[1]<-"radiancedeath"
ex4e_g130_h0<-merge( ex4e_g130_h0, ex4e_g130_death, by.x=8, by.y= 2)
ex4e_g130_h0test<-cor.test(ex4e_g130_h0$radiance, ex4e_g130_h0$radiancedeath)

FIGS9a <-ggplot(data = ex4e_g130_h0, aes(x=`radiancedeath`,y=`radiance`))

FIGS9a +
  geom_point()+
  ylab("initial fLux (p/s)")+
  xlab("flux at death (p/s)")+
  annotate("text", y=600000, x=3000000,label=paste("Pearson's product-moment\n correlation\nR=",signif(ex4e_g130_h0test$estimate,2),"\np=",signif(ex4e_g130_h0test$p.value,2) ,"\n CI=",signif(ex4e_g130_h0test$conf.int[1],2),",",signif(ex4e_g130_h0test$conf.int[2],2) ),size=3)+
  #annotate("text", x="G130", y=60000000,label=paste("n.s."),size=3)+
  ggtitle("Supplemental Figure 9a\nimd10191 initial vs final dose ")+
  scale_y_continuous(breaks= ax_breaks, labels=ax_labeles, trans='log',limits=c(10000,1000000))+
  scale_x_continuous(breaks= ax_breaks,labels=ax_labeles,trans='log',limits= c(1000000,100000000))+
  theme_bw()+
  theme(axis.title.x = element_text(size = 10), 
        axis.title.y = element_text(size = 10),
        axis.text.x = element_text(size= 10), 
...
ex4e_g90_h0<-fig7a[fig7a$genotype=="G090" & fig7a$Hour==0,]
ex4e_g90_h48<-fig7a[fig7a$genotype=="G090" & fig7a$Hour==48,]
ex4e_g90_h48<-ex4e_g90_h48[,c("radiance","ID")]
colnames(ex4e_g90_h48)[1]<-"final_rad"
ex4e_g090_if<-merge( ex4e_g90_h0, ex4e_g90_h48, by.x=8, by.y= 2)
ex4e_g090_ifTEST<-cor.test(ex4e_g090_if$radiance, ex4e_g090_if$final_rad)

FIGS9b <-ggplot(data = ex4e_g090_if, aes(x=`final_rad`,y=`radiance`))

FIGS9b +
  geom_point()+
  ylab("initial fLux (p/s)")+
  xlab("final flux (p/s)")+
  annotate("text", y=400000, x=55000,label=paste("Pearson's product-moment\n correlation\nR=",signif(ex4e_g090_ifTEST$estimate,2),"\np=",signif(ex4e_g090_ifTEST$p.value,2) ,"\n CI=",signif(ex4e_g090_ifTEST$conf.int[1],2),",",signif(ex4e_g090_ifTEST$conf.int[2],2) ),size=3)+
  #annotate("text", x="G130", y=60000000,label=paste("n.s."),size=3)+
  ggtitle("Supplemental Figure 9b\n WT initial vs final dose ")+
  scale_y_continuous(breaks= ax_breaks, labels=ax_labeles, trans='log',limits= c(10000,1000000))+
  scale_x_continuous(breaks= ax_breaks,labels=ax_labeles,trans='log',limits= c(1000,100000))+
  theme_bw()+
  theme(axis.title.x = element_text(size = 10), 
        axis.title.y = element_text(size = 10),
        axis.text.x = element_text(size= 10), 
...
#### figure S13
```{r}
ex4e_g90_h0<-ex4e_90_rehand[ex4e_90_rehand$genotype=="G090" & ex4e_90_rehand$Hour==0,]
ex4e_g90_h115<-ex4e_90_rehand[ex4e_90_rehand$genotype=="G090" & ex4e_90_rehand$Hour==48,c("radiance","ID")]
colnames(ex4e_g90_h115)[1]<-"radiance48"
ex4e_g90_h0<-merge( ex4e_g90_h0, ex4e_g90_h115, by.x=8, by.y= 2)
ex4e_g90_h0test<-cor.test(ex4e_g90_h0[ex4e_g90_h0$clustlabs=="C1", c("radiance")], ex4e_g90_h0[ex4e_g90_h0$clustlabs=="C1", c("radiance48")])
ex4e_g90_h0test2<-cor.test(ex4e_g90_h0[ex4e_g90_h0$clustlabs=="C2", c("radiance")], ex4e_g90_h0[ex4e_g90_h0$clustlabs=="C2", c("radiance48")])

FIGS13a <-ggplot(data = ex4e_g90_h0, aes(x=`radiance48`,y=`radiance`, color=`clustlabs`))

FIGS13a +
  geom_point()+
  ylab("initial fLux (p/s)")+
  xlab("final flux (p/s)")+
  annotate("text", y=20000, x=2000,label=paste("Pearson's product-moment\n correlation (group1)\nR=",signif(ex4e_g90_h0test$estimate,2),"\np=",signif(ex4e_g90_h0test$p.value,2) ,"\n CI=",signif(ex4e_g90_h0test$conf.int[1],2),",",signif(ex4e_g90_h0test$conf.int[2],2) ),size=3)+
  annotate("text", y=20000, x=50000,label=paste("Pearson's product-moment\n correlation (group2)\nR=",signif(ex4e_g90_h0test2$estimate,2),"\np=",signif(ex4e_g90_h0test2$p.value,2) ,"\n CI=",signif(ex4e_g90_h0test2$conf.int[1],2),",",signif(ex4e_g90_h0test2$conf.int[2],2) ),size=3)+
  #annotate("text", x="G130", y=60000000,label=paste("n.s."),size=3)+
  ...
  theme(axis.title.x = element_text(size = 10), 
        ...
        axis.text.x = element_text(size= 10), 
        axis.text.y = element_text(size= 10), panel.grid.minor = element_blank())+
  scale_color_manual(name="Group" ,values = colors_cb, labels = c("group1", "group2"))


ex4e_90_hi<-ex4e_90_rehand[ ex4e_90_rehand$clustlabs=="C1" & ex4e_90_rehand$Hour==0,]
ex4e_90_lo<-ex4e_90_rehand[ ex4e_90_rehand$clustlabs=="C2" & ex4e_90_rehand$Hour==0,]
ex4e_90_inittest<-t.test(ex4e_90_hi$radiance, ex4e_90_lo$radiance)
figs13b<-rbind(ex4e_90_hi, ex4e_90_lo)
...
FIGS13B +
  ...
  ylab("initial flux (p/s)")+
  xlab("group")+
  annotate("text", x=1.5, y=900000,label=paste("n.s p=",signif(ex4e_90_inittest$p.value,2)))+
  ggtitle("Supplemental Figure 13b")+
  scale_y_continuous(breaks= ax_breaks,labels=ax_labeles,trans='log',limit=c(10000,1000000))+
  theme_bw()
...
figs13c<-rbind(ex4e_90c1, ex4e_90c2)

FIGS13C <-ggplot(data = figs13c, aes(x=`clustlabs`, y=`radiance`))
...
FIGS13C +
  ...
  ggtitle("Supplemental Figure 13c")+
